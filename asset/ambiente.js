...
btnConverter.addEventListener('click', function () {
  real = parseFloat(valor.value)
  //verificando qual o indisse  foi selecionado
  for (let i = 0; i < moeda.length; i++) {
    if (moeda[i].checked) {
      moedaEstrangeira = moeda[i].value
    }
  }
  //com o indice selecionando faz a conta baseada no valor
  switch (moedaEstrangeira) {
    case 'Dolar':
      real / dolar
      mensagemFormatada(
    ...
    case 'Baht':
      moedaConvertida = real / valorDaLibra
      mensagemFormatada(
        moedaConvertida.toLocaleString('en-Thailand', {
          style: 'currency',
          currency: 'THB'
        })
      )
    ...
    case 'Biticoin':
      moedaConvertida = real / biticoin
      mensagemFormatada(parseFloat(moedaConvertida).toFixed(5))
      break

    default:
      resultado.textContent = 'Escolha uma moeda estrangeira'
  }
  isNaN(moedaConvertida) ? (moedaConvertida = 0) : ''
})
